...
The 4Ever AI Life OS Platform uses **npm** as its primary package manager across three independent projects (backend, frontend, mobile), each maintaining separate `package.json` and `package-lock.json` files. There is no workspace orchestration tool (e.g., npm workspaces, Lerna, Nx, or pnpm) — each project manages dependencies independently.
