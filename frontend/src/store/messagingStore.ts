...
import { connectionsApi, messagesApi, type ConnectionItem, type PendingRequest, type ConversationPreview, type DirectMessage, type TriChatStatus } from '../api/messaging'
...
interface MessagingState {
  ...
  // Tri-Chat Mediator (per active conversation)
  triChat: TriChatStatus | null
  triChatLoading: boolean
  mediatorStreamingId: string | null // placeholder messageId currently streaming
...
export const useMessagingStore = create<MessagingState>((set, get) => ({
  ...
  openChat: async (userId: string, name: string, connectionId?: string) => {
    ...
    try {
      ...
      // Keep totalUnread in sync with server truth + clear per-conversation badge
      get().loadUnreadCount()
      get().markChatRead(userId)
      // Load tri-chat status if we have a connectionId
      if (connectionId) {
  ...
  addIncomingMessage: (msg: DirectMessage) => {
    ...
    // A message belongs to the active chat if:
    //  - it's FROM the person we're chatting with, OR
    //  - it's TO the person we're chatting with and was authored by us (e.g.
    //    mediator placeholder where senderId = summoner = self). This ensures
    //    the summoner sees their own 4Ever bubble stream in real time.
    ...
    if (belongsToActiveChat) {
      ...
      // increment unread only for messages truly authored by someone else
  ...
  replaceTempWithReal: (realMsg: DirectMessage) => {
    set((s) => {
      ...
      // If no temp found, check if real message already exists (deduplicate)
      const exists = s.chatMessages.some((m) => m.id === realMsg.id)
      if (!exists) {
        return { chatMessages: [...s.chatMessages, realMsg] }
      }
      return {}
    })
  },
  ...
  finishMediatorStream: (messageId: string, patch?: { actions?: string | null; sessionId?: string }) => {
    set((s) => ({
      ...
      chatMessages: patch?.actions !== undefined
        ...
        : s.chatMessages,
      // Decrement turnsLeft locally when the stream the user started finishes.
  ...
  applyChatHistoryCleared: (connectionId: string) => {
    // The user just cleared their own view. Wipe the local transcript + mark
    // hasClearedHistory so the UI can show a fresh-slate state. The other
    // party is unaffected; their socket will not receive this event.
    set((s) => ({
      chatMessages:
  ...
  applyMediatorRenamed: (connectionId: string, mediatorName: string) => {
    // Shared rename: both parties receive `mediator_renamed`. Update the
    // active triChat state so the UI reflects the new name without a full
    // status refetch.
    set((s) => ({
      triChat:
