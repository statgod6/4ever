...
export interface RelationshipHealthData {
  ...
  driftingPeople: {
    ...
    lastInteractionAt: string | null; interactionCount: number; daysSinceInteraction: number | null
  }[]
  peopleWithScores: { id: string; name: string; relationship: string; healthScore: number }[]
  recentActivity: {
    id: string; personName: string; personRelationship: string;
    content: string; sentiment: string | null; topic: string | null; source: string; createdAt: string
  }[]
}
...
export interface AnnualReviewData {
  ...
  mostActive: {
    id: string; name: string; relationship: string; noteCount: number;
