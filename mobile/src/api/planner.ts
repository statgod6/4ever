...
export interface PlanTask {
  id: string; timeSlot: string; task: string; insight: string | null;
  status: string; completedAt: string | null; sortOrder: number
}
export interface DayPlan { id: string; date: string; tasks: PlanTask[] }
export interface TaskInsightResult { taskId: string; insight: string; cached: boolean }
export interface PlannedDate { date: string; taskCount: number }
export interface CompletionStats {
  total: number; done: number; skipped: number; pending: number;
  completionRate: number; streak: number; days: number
}
